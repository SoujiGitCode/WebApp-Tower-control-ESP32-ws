...
const Graph = ({
  onBack,
  webSocketAdress = "ws://192.168.4.1:8080",
  devMode = false,
}) => {
  ...
  const saveChanges = () => {
    ...
    if (chartInstanceRef.current) {
      ...
      chart.options.scales.y.max =
        chartMax > 0 ? chartMax : chart.options.scales.y.max;
      chart.update();
    }
  ...
  const updateLedStatus = async (ledStatus) => {
    try {
      ...
      if (response.status === 200) {
        ...
        console.error(
          "Error al actualizar el estado del LED:",
          response.statusText
        );
      }
    } catch (error) {
      console.error(
        "Error en la solicitud para actualizar el estado del LED:",
        error
      );
    }
  };
  ...
  const checkForOutOfBounds = (history) => {
    ...
    Object.entries(averageValues).forEach(([key, average]) => {
      if (average < min) {
        outOfBoundsMessages.push(
          `Celda ${key} por debajo del mínimo permitido: ${average.toFixed(
            2
          )} < ${min}`
        );
        ledShouldBeOn = true;
        ...
        outOfBoundsMessages.push(
          `Celda ${key} por encima del máximo permitido: ${average.toFixed(
            2
          )} > ${max}`
        );
        ledShouldBeOn = true;
    ...
    if (ledShouldBeOn) {
      ...
      console.log(
        "LED encendido, valores fuera de rango detectados:",
        outOfBoundsMessages
      );
    } else {
      ...
      try {
        ...
        if (now - lastUpdate >= interval * 1000) {
          ...
          setValueHistory((prevHistory) => {
            ...
            if (
              Object.values(updatedHistory).every(
                (arr) => arr.length === valueCount
              )
            ) {
              console.log(
                `Máxima cantidad de valores alcanzada (${valueCount}). Evaluando...`
              );
    ...
    >
      ...
      <Modal open={modalOpen} onClose={cancelChanges}>
        ...
        >
          ...
          >
            Ajusta el intervalo de actualización y la cantidad de valores que se
            almacenan.
          </Typography>
          <TextField
            fullWidth
            ...
            onChange={(e) =>
              handleTempChange("interval", Number(e.target.value))
            }
          ...
          <TextField
            ...
            onChange={(e) =>
              handleTempChange("valueCount", Number(e.target.value))
            }
            onKeyDown={handleKeyDown}
            sx={{ mb: 2 }}
          />
          ...
          >
            Ajusta el valor máximo del eje Y para representar los datos de forma
            clara.
          </Typography>
          <TextField
            fullWidth
            ...
            onChange={(e) =>
              handleTempChange("chartMax", Number(e.target.value))
            }
            onKeyDown={handleKeyDown}
            sx={{ mb: 2 }}
          />
